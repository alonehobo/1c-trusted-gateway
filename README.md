...
```
trusted-gateway-go/
├── main.go              # Точка входа, открытие браузера, запуск сервера
├── tray_windows.go      # Системный трей Windows (Shell_NotifyIcon, balloon, flash)
├── web.go               # TrustedWebApp: состояние приложения, бизнес-логика
├── http_handlers.go     # HTTP-сервер, все API-эндпоинты, SSE, маршрутизация
├── helpers.go           # Утилиты: парсинг JSON-полей, CSV, санитизация импорта
├── mcp_server.go        # MCP-сервер (Streamable HTTP, JSON-RPC 2.0)
├── service.go           # Runtime: выполнение запросов, маскировка, bundle
...
├── query_normalize.go   # Нормализация псевдонимов КАК/AS в запросах
