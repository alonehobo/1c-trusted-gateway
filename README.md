...
| **Настройки** | URL, токен, белый список полей, **политика типов**, авто-режим (зашифрованы через Windows DPAPI) |
...
### Слой 1 — Type-aware маскировка (основной)

Шлюз получает схему колонок из 1С (типы метаданных) и применяет политику по типам:

| Тип колонки | Действие по умолчанию |
|-------------|------------------------|
| `Перечисление.*`, `ПланСчетов.*`, `ПланВидовХарактеристик.*` | Plain (открыто) |
| Платформенные enum'ы без точки (`ВидДвиженияНакопления`, `ВидСчета` и т.п.) | Plain |
| Примитивы: `Число`, `Дата`, `Булево`, `УникальныйИдентификатор` | Plain |
| Классификаторы: `Справочник.Валюты`, `СтавкиНДС`, `СтраныМира`, `БанкиРФ` | Plain |
| `Справочник.*`, `Документ.*`, `Задача.*`, `БизнесПроцесс.*` | Mask |
| `Строка`, `ХранилищеЗначения`, `ДвоичныеДанные` | Mask |
| Составные типы с обрезанием (>10 вариантов) | Mask (на случай скрытых опасных типов) |
| Колонка с несколькими типами, где хоть один — опасный | Mask |

Политика настраивается в разделе **«Настройки → Политика типов»**: можно добавлять свои типы/префиксы, переводить их в plain или forced-mask.

### Слой 2 — Стрип `ПРЕДСТАВЛЕНИЕ()` (автоматически)

Шлюз перед отправкой в 1С вычищает `ПРЕДСТАВЛЕНИЕ(X)` / `ПРЕДСТАВЛЕНИЕССЫЛКИ(X)` / `PRESENTATION(X)` / `REFPRESENTATION(X)`, подставляя на их место аргумент. Это позволяет сохранить тип колонки (ссылка, а не строка) и применить type-aware маскировку вместо безусловного Mask для `Строка`.
...
### Слой 3 — Эвристики для значений (fallback)
...
- **Нормализация псевдонимов** — `КАК/AS` в запросе приводится к имени исходного поля, чтобы агент не смог обойти whitelist переименованием
...
├── privacy_test.go      # Тесты type-aware маскировки
...
├── ui.html              # Весь фронтенд (HTML/CSS/JS + Ace Editor)
